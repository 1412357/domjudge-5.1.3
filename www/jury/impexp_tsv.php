...
function tsv_import($fmt)
{
	...
	switch ($fmt) {
		...
		case 'accounts':
			...
			break;
		case 'courses':
			$data = tsv_courses_prepare($content);
			$c = tsv_courses_set($data);
			break;
...
function tsv_courses_prepare($content) {
	$data = array();
...
function tsv_courses_set($data) {
	global $DB;

	$courses = array();
	// Check all courses and users exist
	foreach($data as $courseName => $users) {
		try {
			$courseId = $DB->q("VALUE SELECT courseid FROM course WHERE name = %s", $courseName);
			$courses[$courseId] = array();
			add_teams_set($data[$courseName]);
			$courses[$courseId] = add_accounts_set($data[$courseName]);
		} catch (Exception $e) {
			error($courseName . " course doesn't exist or " . $e->getMessage());
		}
	}
	$c = 0;
	...
	foreach($courses as $courseId => $users) {
		foreach($users as $user) {
			try {
				$userId = $user["userid"];
				$teamId = $user["teamid"];
				$DB->q("replace into course_user(userid, courseid) values(%i, %i)", $userId, $courseId);
				addUserToContestTeamTable($courseId, $teamId);
				$c++;	
			} catch (Exception $e) {
				echo $e->getMessage();
			}
...
function add_accounts_set($data)
{
	global $DB;
	$users = array();
	foreach ($data as $row) {
		$row['user']["teamid"] = $DB->q('VALUE SELECT teamid FROM team WHERE name = %s', $row['user']['username']);
		$values = $row['user'];
		$DB->q("INSERT INTO user(username, name, password, email, teamid) 
				VALUES('$values[username]', '$values[name]', '$values[password]', '$values[email]', $values[teamid]) 
				ON duplicate key UPDATE %S", $row['user']);
...
function add_teams_set($data)
{
	return tsv_teams_set($data);
}
...
function tsv_teams_prepare($content)
{
	...
	foreach($content as $line) {
		...
		// Delete 2 first column teamid, externalid
		$data[] = array (
			'team' => array (
				'categoryid' => @$line[0],
				'name' => @$line[1]),
...
function tsv_teams_set($data)
{
	...
	foreach ($data as $row) {
		...
		if ( !empty($row['team_affiliation']['shortname']) ) {
			$values = $row['team_affiliation'];
			$DB->q("INSERT INTO team_affiliation(shortname, name, country) 
					VALUES('$values[shortname]', '$values[name]', '$values[country]') 
					ON duplicate key UPDATE %S", $row['team_affiliation']);
		...
		$values = $row['team'];
		$DB->q("INSERT INTO team(name, categoryid)
				VALUES('$values[name]', '$values[categoryid]') 
				ON duplicate key UPDATE %S", $row['team']);
...
function tsv_accounts_set($data)
{
	...
	foreach ($data as $row) {
		...
		$values = $row['user'];
		$DB->q("INSERT INTO user(username, name, password) 
				VALUES('$values[username]', '$values[name]', '$values[password]') 
				ON duplicate key UPDATE %S", $row['user']);
		$userid = $DB->q("VALUE SELECT userid FROM user WHERE username = %s", $row['user']['username']);
		auditlog('user', $userid, 'replaced', 'imported from tsv');
		$row['userrole']['userid'] = $userid;
