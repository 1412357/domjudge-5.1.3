...
$submitThroughFile = true;
if (empty($_POST["maincode"])) {
	$submitThroughFile = false;	// flag submit throught editor
	$scode = $_POST["mainEditorCode"];
	// Create temporary file to store editor source code
	$tmpfname = tempnam("./tmp", "foo");
	$handle = fopen($tmpfname, "w");
	fwrite($handle, $scode);
	fclose($handle);

	$mock_json = '{"code":{"name":["A.cpp"],"type":["text\/plain"],"tmp_name":["'.$tmpfname.'"],"error":[0],"size":['.filesize($tmpfname).']}}';
	$_FILES = json_decode($mock_json, true);
}
