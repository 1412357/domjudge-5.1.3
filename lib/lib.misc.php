 ...
 */
function submit_solution($team, $prob, $contest, $lang, $files, $filenames, $origsubmitid = NULL, $submitThroughFile = true)
{
	global $DB;
	...
	if ( is_writable( SUBMITDIR ) ) {
		...
		if (!$submitThroughFile) {	//  if submit through editor, so clear all temporary file
			for($rank=0; $rank<count($files); $rank++) {
				unlink($filenames[$rank]);
			}
		}
	} else {
