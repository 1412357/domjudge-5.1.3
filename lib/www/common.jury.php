...
function addUserToContestTeamTable($courseId, $teamId) {
	global $DB;
	$contestIDs = $DB->q("TABLE select contestid from lecture where courseid = $courseId;");
	foreach ($contestIDs as $contestID) {
		$DB->q("replace into contestteam(cid, teamid) values(%i, %i)", $contestID['contestid'], $teamId);
	}
}
