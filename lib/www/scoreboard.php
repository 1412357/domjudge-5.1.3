...
function putTopRated() {
	global $DB;
	$res = $DB->q('table select username, totalcorrect, teamid, userid
				from (
					select teamid, sum(is_correct) as totalcorrect 
					from scorecache_public
					where is_correct = 1
					group by teamid ) as team_correct
				inner join user using(teamid)
				order by totalcorrect desc
				limit 10');
	echo '
	<table class="common-block table-striped table-bordered">
		<caption class="common-block-header pl-3 pt-1" style="height: 30px">Top rated</caption>
		<thead>
		<tr>
			<th class="">#</th>
			<th class="">Top rated</th>
			<th class="">Rating</th>
		</tr>
		</thead>
		<tbody>';
		...
		foreach ($res as $row) {
			...
			echo '<tr>
					...
					<td style="color: red; font-weight: bold;">'.$row['username'].'</td>
...
function putTopRatedCourse($courseid) {
	global $DB;
	$res = $DB->q('table select username, totalcorrect, teamid, userid
					from (
					select teamid, sum(is_correct) as totalcorrect 
					from scorecache_public
					where is_correct = 1 and cid in (select contestid
													from lecture
													where courseid = %i)
													group by teamid ) as team_correct
					inner join user using(teamid)
